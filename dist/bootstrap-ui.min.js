...
this.change=function(t){if(!t){return s}if(s){this.removeClass("glyphicon-"+s)}this.addClass("glyphicon-"+t);s=t;return this};if(r){this.change(r)}}})(k);(function(p){var s=p.InputList=g("InputList",n);
s.hook("attr","name","set",function(t){this.$list.find("input").attr("name",t+"[]");return t});function r(v,w,u){var t=$("<li/>");$("<span/>").addClass("bui-label").text(w?w:v).appendTo(t);$("<input/>").attr({type:"hidden",name:u+"[]"}).val(v).appendTo(t);
$("<a/>").addClass("bui-delete").appendTo(t);return t}function q(z,w,t){if(z.constructor!==Array){throw new TypeError("$bui.InputList.val() 参数必须是数组。")}var x=[];var y;for(var v=0,u=z.length;v<u;v++){if(w[z[v]]){y=w[z[v]]
}x[v]=r(z[v],y,t)[0]}return $(x)}function o(t){t.stopPropagation()}function n(){var y=this;var z=[];var u={};var w=$('<ul class="list"/>').appendTo(this);var v=(new p.FormControl()).appendTo($('<div class="control"/>').appendTo(this));
var x;var t=new p.Button(new p.Icon("plus"));this.$list=w;v.append(t);this.centerWidget=function(C){if(x){x.off("keydown",B);x.off("change",o)}x=v.centerWidget(C);x.attr("name","");x.on("keydown",B);x.on("change",o)
};this.centerWidget($("<input/>").attr("type","text"));this.on("click",".bui-delete",function(E){var C=$(this).prev().val();var D=z.indexOf(C);if(D>-1){z.splice(D,1)}$(this).parent().remove();E.preventDefault();
l(y,z)});function B(C){if(C.shiftKey||C.ctrlKey||C.altKey){return}if(C.which==13){var D=x.val();x.val("").focus();A(D);return false}}this.val=function(C){if(arguments.length==0){return z}else{var D=y.attr("name");
z=C;var E=q(C,u,D);E.appendTo(w.empty());return this}};t.on("click",a("left",function(){var C=x.val();x.val("").focus();A(C)}));this.addVal=A;function A(E,D){if(!E){return false}if(!D){if(u[E]){D=u[E]}}if(y.data("unique")&&z.indexOf(E)>=0){return false
}var C=y.attr("name");r(E,D,C).appendTo(w);z.push(E);l(y,z)}this.mapTitle=function(C,D){if(D===undefined||D){$.extend(u,C)}else{u=C}return this}}})(k);(function(p){var o=p.IntInput=g("IntInput",n);o.hook("attr","range","get",function(){return this.range.toString()
...
G.popover("show")}if(L.error){x="上传失败";G.popover("show")}else{u()}}).on("fileuploadprogressall",function(L,K){var M=parseInt(K.loaded*1000/K.total,10)/10;y.attr("progress",M)}).on("fileuploaddone",function(M,L){var K=new $.Event("upload_result");
z.trigger(K,[L.result]);if(K.isDefaultPrevented()){z.val(D.current);if(!K.isPropagationStopped()){x="上传失败";G.popover("show")}}else{z.val(L.result.url);F.popover("show")}r()}).on("fileuploadfail",function(L,K){x="发生网络错误，上传失败，请重试。";
G.popover("show");r()});var w=0;this.doUpload=s;this.getInstance=function(){return J};r();function s(K){if(A){return}J.submit();J.jqXHR.url=J.url;LogStandardReturn(J.jqXHR,K?K:"单个图片文件上传");q()}function r(){G.popover("hide");
E.addClass("disabled");A=true;v.addClass("disabled");t.val("");y.attr("progress",0);w=0}function u(){E.removeClass("disabled");A=false;v.removeClass("disabled");w=1}function q(){x=null;G.popover("hide");
E.addClass("disabled");A=true;v.removeClass("disabled");w=2}}})(k)})(window);
...
};this.$input.setValue=function(K){return H=K};var t=I.centerWidget($("<input/>").attr("type","file"));var y=new o.Progress();y.addClass("progress-striped active").css({width:"200px"});I.append(y);var F=$("<span/>").append(new o.Icon("eye-open"));
var C=$('<div><img width="100" height="100"/><div style="width:100px;height:100px;text-align:center;">无</div></div>');I.prepend(F);F.popover({html:true,title:"当前图片",content:function(){C.find("img").attr("src",H);
...
var J={};var A=true;var E=(new o.Button(new o.Icon("cloud-upload"))).click(function(){s()});var v=(new o.Button(new o.Icon("remove"))).click(function(){if($(this).hasClass("disabled")){return}if(w==2){J.abort()
}r()});I.append(E);I.append(v);t.fileupload(D).on("fileuploadadd",function(L,K){u()}).on("fileuploadprocessalways",function(N,M){J=M;var K=M.index;var L=M.files[K];if(!L){return r()}if(L.preview){x=L.preview;

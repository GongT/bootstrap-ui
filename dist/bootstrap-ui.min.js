...
}});(function(n){var o=n.Progress=g("Progress",p);o.hook("attr","progress","set",function(q){this._bui_bar._setvalue(q);this._progress=q});o.hook("attr","progress","get",function(){return this._progress
});function p(){this.addClass("progress");var q=$('<div class="progress-bar"  role="progressbar"/>').appendTo(this);var r=$('<span class="sr-only"/>').appendTo(q);q._setvalue=function(t){q.css("width",t+"%");
r.text("已经完成百分之"+t)};this._progress=0;this._bui_bar=q;var s="";this.alert=function(t){if(s){this.removeClass(s);s=""}if(t){s="progress-bar-"+t;this.addClass(s)}}}})(k);(function(o){var n=o.Select=g("Select",s);
n.proxyInput=true;n.init=function(){$(document).on("click",p)};n.hook("attr","title","set",function(t){if(this.$input&&!this.$input.val()){this.$show.text(t)}return t});var r=null;function p(){if(r){r.removeClass("active");
r=null}}function q(u,t){return $("<li/>").attr("value",t).append($("<a/>").html(u))}function s(){var u={};var w=this.addClass("form-control btn btn-default");var t=$('<ul class="dropdown-menu" role="menu"/>');
this.addOption=function(y,x){u[x]=y;t.append(q(y,x))};this.find("option").each(function(){w.addOption($(this).val(),$(this).text());$(this).remove()});t.append(this.children()).appendTo(this);var v=$('<div class="bui-toggle"/>').prependTo(w);
w.$show=$("<span/>").appendTo(v);$('<span class="caret"/>').appendTo(v);w.$input=$("<input/>").attr("type","hidden").val("").prependTo(w);w.$input.setValue=function(x){if(!u.hasOwnProperty(x)){w.$show.text(w.attr("title"));
return""}w.$show.text(u[x]);return x};if(!this.attr("title")){this.attr("title","请选择")}w.click(function(){if(w.hasClass("active")){p()}else{p();r=w.addClass("active")}return false});t.on("click","li",function(){var x=$(this).attr("value");
w.$input.val(x);w.$show.text($(this).text());p();l(w,x);return false})}})(k);(function(p){var o=p.Switch=g("Switch",n);o.proxyInput=true;o.hook("attr","disabled","set",function(r){if(typeof r==="string"){r=r.toLowerCase()=="disabled"
}if(r){this.$btn.addClass("disabled");this.addClass("disabled")}else{this.$btn.removeClass("disabled");this.removeClass("disabled")}this._lock=r;return r});function q(r){r=b(r);$(this)[(r?"add":"remove")+"Class"]("on");
if(r){this.$input.removeAttr("disabled")}else{this.$input.attr("disabled","disabled")}this.current_status=r;return false}function n(s){var w=this;this.current_status=!!s;this.icon=new p.Icon("");this.$input=$("<input/>").val("true").attr("type","hidden").appendTo(this);
this.$input.setValue=function(A){A=b(A);w[(A?"add":"remove")+"Class"]("on");w.current_status=A;return A?"true":"false"};this.$input.getValue=function(){return w.current_status};var u=$("<div/>").addClass("bui-switch-container").appendTo(this);
...
return}if(w._lock){return}w.val(!w.current_status);l(w,w.current_status)})}})(k);(function(o){var n=o.UploadSingle=g("UploadSingle",p);n.proxyInput=true;function p(B){if(typeof B==="string"){B={url:B}}if(!B||!B.url){throw new Error("bui.UploadSingle 构造必须指定一个url")
}B=$.extend({dataType:"json",autoUpload:false,acceptFileTypes:/\.(gif|jpe?g|png)$/i,maxFileSize:3000000,disableImageResize:/Android(?!.*Chrome)|Opera/.test(i.navigator.userAgent),previewMaxWidth:100,previewMaxHeight:100,maxChunkSize:undefined,multipart:true,previewCrop:true,current:null,formData:function(){return[]
}},B);var G=(new o.FormControl()).appendTo(this);var F=B.current;var z=this;this.$input=$('<input style="display: none!important;"/>').attr("type","text").appendTo(this);this.$input.getValue=function(){return F
};this.$input.setValue=function(I){return F=I};var t=G.centerWidget($("<input/>").attr("type","file"));var y=new o.Progress();y.addClass("progress-striped active").css({width:"200px"});G.append(y);var D=$("<span/>").append(new o.Icon("eye-open"));
G.prepend(D);D.popover({html:true,title:"当前图片",content:function(){return F?'<img src="'+F+'" width="100" height="100"/>':'<div style="width:100px;height:100px;text-align:center;">无</div>'},placement:"auto",trigger:"hover"});
var x="";var E=t.parent().popover({html:true,title:"选中的图像",content:function(){return x?x:"没有选中"},placement:"auto bottom",trigger:"manual hover"});var H={};var A=true;var C=(new o.Button(new o.Icon("cloud-upload"))).click(function(){r()
});var v=(new o.Button(new o.Icon("remove"))).click(function(){if($(this).hasClass("disabled")){return}if(w==2){H.abort()}s()});G.append(C);G.append(v);t.fileupload(B).on("fileuploadadd",function(J,I){u()
}).on("fileuploadprocessalways",function(L,K){H=K;var I=K.index;var J=K.files[I];if(!J){return s()}if(J.preview){x=J.preview;E.popover("show")}if(J.error){x="上传失败";E.popover("show")}else{u()}}).on("fileuploadprogressall",function(J,I){var K=parseInt(I.loaded*1000/I.total,10)/10;
y.attr("progress",K)}).on("fileuploaddone",function(J,I){if(I.result&&I.result.success){z.val(I.result.url);D.popover("show")}else{z.val(B.current);x="上传失败";E.popover("show")}s()}).on("fileuploadfail",function(J,I){x="发生网络错误，上传失败，请重试。";
E.popover("show");s()});var w=0;this.doUpload=r;this.getInstance=function(){return H};s();function r(I){if(A){return}H.submit();H.jqXHR.url=H.url;LogStandardReturn(H.jqXHR,I?I:"单个图片文件上传");q()}function s(){E.popover("hide");
C.addClass("disabled");A=true;v.addClass("disabled");t.val("").removeAttr("disabled");y.attr("progress",0);w=0}function u(){C.removeClass("disabled");A=false;v.removeClass("disabled");t.attr("disabled","disabled");
w=1}function q(){x=null;E.popover("hide");C.addClass("disabled");A=true;v.removeClass("disabled");t.attr("disabled","disabled");w=2}}})(k)})(window);